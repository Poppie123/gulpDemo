...
var parseQueryString = function (url) { //获取参数链接转为json    传的参数格式例如  ajax  id=1&msg=2
    reg_para = /([^&=]+)=([\w\W]*?)(&|$|#)/g,
        ret = {};
    var str_para = url,
        result;
...
//封装请求
var ajaxData = function(purl,data,callback,methods){
  api.ajax({
    url: url+purl,
    method: methods  || 'get',
    data:{
...
function getnodata(text1,src1){
  function NodataLayer(text,src){
    ...
    this.render = function(){
          var str = '<div class="no_data"><img src="'+imgSrc+'"/>'
                   +'<div class="no_data_text">'+tips+'</div></div>'
